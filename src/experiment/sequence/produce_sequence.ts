import { Version, Position, CongruencyMatrix } from "./components/constants";
...
import { randomSampleSequencer, randomRepetitionLockedSequencer } from "./util/sequencer";

export function produce_sequence(group_nr: number, block_type: string, block_nr: number): TrialData[] {
    const group = GROUPS[group_nr];

    function add_data(position: Position, context: ContextInfo) {
        return function (trial: ContextData, context_trial_nr: number): ContextData {
            // Function
